...
describe('MemecoinsController', () => {
  ...
  describe('getMemecoinsByCreator', () => {
    it('should return memecoins created by a specific user', async () => {
      const creatorId = mockUser._id.toString();
      const result = await controller.getMemecoinsByCreator(creatorId);
      
      expect(service.findByCreator).toHaveBeenCalledWith(creatorId);
      expect(result).toEqual([mockMemecoin]);
    ...
    it('should return empty array for creator with no memecoins', async () => {
      ...
      const result = await controller.getMemecoinsByCreator(new Types.ObjectId().toString());
  ...
  it('should protect createMemecoin with JwtAuthGuard', () => {
    ...
    it('should not protect getAllMemecoins with any guard', () => {
